...
export default function ExerciseCard(props) {
    const { exercise, i } = props // Destructure exercise and index from props
    const [setsCompleted, setSetsComplete] = useState(0) // State to track completed sets

    // Function to increment the number of sets completed
    function handleSetIncrement() {
        setSetsComplete((setsCompleted + 1) % 6) // Increments completed sets, resets after 5
    }

    return (
        // Card that displays exercise details
        <div className='p-4 rounded-md flex flex-col gap-4 bg-slate-950 sm:flex-wrap'>
            ...
            <div className='flex flex-col bg-slate-950 rounded gap-2 '>
                {exercise.description.split('___').map((val, index) => { // Split description into parts
                    return (
            ...
            <div className='grid grid-cols-2 sm:grid-cols-4 sm:place-items-center gap-2'>
                {['reps', 'rest', 'tempo'].map(info => { // Display exercise info
                    return (
                ...
                <button onClick={handleSetIncrement} className='flex flex-col p-2 rounded border-[1.5px] border-solid border-blue-900 hover:border-blue-600 w-full duration-200'>
                    ...
                    <p className='font-medium'>{setsCompleted} / 5</p> {/* Display sets completed */}
                </button>
            </div>
        </div>
    )
